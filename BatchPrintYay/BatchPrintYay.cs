...
namespace BatchPrintYay.Model
{
    [Autodesk.Revit.Attributes.Transaction(Autodesk.Revit.Attributes.TransactionMode.Manual)]

    class BatchPrintYay : IExternalEventHandler
        {
        public string GetName() => nameof(BatchPrintYay);
        public void TaskDialogMessage(string message)
        {
            var newdialog = new TaskDialog("BatchPrintYay");
        ...
        public void Execute(UIApplication uiapp)
            ...
            Dictionary<string, List<MySheet>> allSheets = SheetSupport.GetAllSheets(uiapp, printSettings);
            ...
            if (!sheetsIsChecked)
            {
                TaskDialogMessage("Не выбраны листы. Выберите листы в Диспетчере проекта через Shift.");
            ...
            */
            ...
            foreach (string docTitle in allSheets.Keys)
            {
                ...
                else
                {
                    ...
                    foreach (Document testOpenedDoc in uiapp.Application.Documents)
                    {
                    ...
                    if (openedDoc == null)
                    {
                        ...
                        if (linkDoc.IsWorkshared)
                        {
                            ...
                            openedDoc = uiapp.Application.OpenDocumentFile(mpath, oo);
                        }
                        else
                        {
                            ...
                            rlt.Unload(new SaveCoordinates());
                            openedDoc = uiapp.Application.OpenDocumentFile(docPath);
                        }
                    }
                ...
                {
                    TaskDialogMessage(formatsCheckinMessage);
                    Debug.WriteLine("Проверка форматов листов неудачна: " + formatsCheckinMessage);
                }
            ...
            if (printSettings.mergePdfs)
            {
                ...
                global::BatchPrintYay.pdf.PdfWorker.CombineMultiplyPDFs(pdfFileNames, combinedFile);
